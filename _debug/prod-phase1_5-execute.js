/**
 * PHASE 1.5 EXECUTE — production /products/ category cleanup.
 * THIS WRITES TO PRODUCTION.
 *
 * Same logic as the dry-run. Rules (locked per Cynthia):
 *   - Clipper's MASTER_PRODUCT_CATEGORIES is the authoritative whitelist
 *   - "Mirrors & Accessories" stays as-is (CCH extended whitelist)
 *   - "Inventory", "Bar", proper names → ROOMS
 *   - Service/expense/freight items → flagged for separate removal (NOT touched here)
 *   - Cant-map values → leave alone
 */
const fs = require('fs');
