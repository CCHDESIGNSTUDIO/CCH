/**
 * 7225 Bugletrail — selections sync execute (STAGING).
 *
 * Writes ONLY to boards/7225-bugletrail/clips.
...
function buildFieldPatch(target, matchedProduct) {
  const patch = {};
  const preview = [];
...
async function main() {
  ...
  const report = {
    script: 'bugletrail-selections-sync-execute.js',
    environment: 'staging',
    boardId: BOARD_ID,
    startedAt,
    protectedSources: [...PROTECTED_SOURCES, 'clip-sync'],
    clipCount: 0,
    candidateCount: 0,
    updatedCount: 0,
    skipped: {
  ...
  const [clipSnap, productsSnap] = await Promise.all([
  ...
  for (const clip of clips) {
    ...
    const linkedProduct = byProductId.get(linkedLibraryProductId(clip)) || null;
    const match = matchItemToProduct(clip, linkedProduct, byHouzzId, bySku, byNameVendor, byNameOnly, byTokenSig);
    if (!match) {
    ...
    if (isDisallowedProductSource(clip.source) || (linkedProduct && isDisallowedProductSource(linkedProduct.source))) {
      report.skipped.protected += 1;
      continue;
    }
    if (!isHouzzLegacyEligible(clip, linkedProduct, match.product)) {
    ...
    const built = buildFieldPatch(clip, match.product);
    const patch = built.patch;
    if (!Object.keys(patch).length) {
      report.skipped.noFieldChanges += 1;
      continue;
    }
    patch._libraryRefreshAt = new Date().toISOString();
    patch._libraryRefreshSource = 'bugletrail-selections-sync-execute';
    patch._libraryRefreshMatchMethod = match.method;
    patch._libraryRefreshMatchedProductId = match.product.id;
    updates.push({
      clipId: clip.id,
      title: String(clip.title || ''),
      room: String(firstNonEmpty(clip, ['roomName', 'room']) || ''),
      patch,
  ...
  for (let i = 0; i < updates.length; i += BATCH_SIZE) {
    ...
    for (const u of slice) {
      batch.update(doc(db, 'boards', BOARD_ID, 'clips', u.clipId), u.patch);
    }
    await batch.commit();
    written += slice.length;
  }

  report.updatedCount = written;
  report.examples = updates.slice(0, 30).map((u) => ({
    clipId: u.clipId,
    title: u.title,
    room: u.room,
    fields: u.fields,
  ...
  const outPath = path.join(__dirname, 'bugletrail-selections-sync-execute-report.json');
